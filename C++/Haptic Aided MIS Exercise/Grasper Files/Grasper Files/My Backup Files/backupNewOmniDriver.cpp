...
void NewOmniDriver::handleEvent(core::objectmodel::Event *event)
{
	if (dynamic_cast<sofa::simulation::AnimateBeginEvent *>(event))
	{
		...
		///////////////////////////////////////////////////////
		//printf("points are == %P \n",tampon);
		//std::cout<<"tampon.getOrigin() = "<<tampon.getOrigin()<<std::endl;
		VecCoord& posD =(*posDevice.beginEdit());
		//std::cout<<"tampon.getOrientation() =  "<<tampon.getOrientation()<<std::endl<<std::endl;
		//std::cout<<"posD[1].getCenter() = "<<posD[1].getCenter()<<std::endl;
		//std::cout<<"posD[1].getOrientation() =  "<<posD[1].getOrientation()<<std::endl;
		//std::cout<<"Z =  "<<posD[1].getCenter()[2]<<std::endl;

		simulation::Node *context1 = dynamic_cast<simulation::Node*>(this->getContext());
//		cerr<<"parent name"<<context1->getParents()<<"\n\n";
		//simulation::Node *context2 = dynamic_cast<simulation::Node*>;
//		cerr<<"check parent name"<<this->parent->name<<"\n\n";
//		cerr<<"check parent's Parent name"<<this->parent->getParents()[0]->name;
		//this->nn
//		cerr<<"parent name"<<context1->getParents()<<"\n\n";
		//context1->getTreeObjects<NewOmniDriver>(&autreOmniDriver);
		//this->parent->getParents()->getTreeObjects<sofa::core::behavior::MechanicalState<sofa::defaulttype::LaparoscopicRigidTypes>, std::vector< sofa::core::behavior::MechanicalState<sofa::defaulttype::LaparoscopicRigidTypes>* > >(&instruments);
		/*cerr<<"my parent name"<<this->parent->getParents()[0]->name<<"\n\n";
		cerr<<"my my parent's child name"<<this->parent->getParents()[0]->getChildren()[0]->name<<"\n\n";*/
		//simulation::Node *context2 = dynamic_cast<simulation::Node*>(this->parent->getParents()[0]->getContext());
		cerr<<"Root == "<<this->parent->getRoot()->name<<"\n\n";
		simulation::Node *context2 = dynamic_cast<simulation::Node*>(this->parent->getRoot());//->getParents()[0]->getContext());

		std::vector< sofa::core::behavior::MechanicalState<sofa::defaulttype::LaparoscopicRigidTypes>* > instruments;
		context2->getTreeObjects<sofa::core::behavior::MechanicalState<sofa::defaulttype::LaparoscopicRigidTypes>, std::vector< sofa::core::behavior::MechanicalState<sofa::defaulttype::LaparoscopicRigidTypes>* > >(&instruments);
		////std::vector< sofa::core::behavior::MechanicalState<sofa::defaulttype::LaparoscopicRigidTypes>* > instruments;
        ////context1->getTreeObjects<sofa::core::behavior::MechanicalState<sofa::defaulttype::LaparoscopicRigidTypes>, std::vector< sofa::core::behavior::MechanicalState<sofa::defaulttype::LaparoscopicRigidTypes>* > >(&instruments);
		//std::cout<<"My child is === "<<context1->getObject("mObject1");
		////cerr<<"\ninstrument exits\n"<<instruments.empty();
        ...
        context1->getTreeObjects<sofa::core::behavior::MechanicalState<sofa::defaulttype::LaparoscopicRigidTypes>, std::vector< sofa::core::behavior::MechanicalState<sofa::defaulttype::LaparoscopicRigidTypes>* > >(&instruments);
		*/
		sofa::core::behavior::MechanicalState<sofa::defaulttype::LaparoscopicRigidTypes>* instrument1 = instruments[0];
		...
		instrumentX[0].getOrientation() = posD[1].getOrientation();// * Quat(Vector3(0,1,0),0.001) * Quat(Vector3(0,0,1),0.001);
		//float Z = posD[1].getOrientation()[2];
		instrumentX[0].getTranslation() = posD[1].getCenter()[2];//tampon.getOrientation();


		//std::vector< sofa::core::behavior::MechanicalState<sofa::defaulttype::RigidTypes>* > rigidinstruments;
        //context1->getTreeObjects<sofa::core::behavior::MechanicalState<sofa::defaulttype::RigidTypes>, std::vector< sofa::core::behavior::MechanicalState<sofa::defaulttype::RigidTypes>* > >(&rigidinstruments);
		
		//sofa::core::behavior::MechanicalState<sofa::defaulttype::RigidTypes>* instrument1 = rigidinstruments[0];

		//helper::WriteAccessor<Data<sofa::defaulttype::RigidTypes::VecCoord> > instrumentX = *instrument1->write(core::VecCoordId::position());//::position());
		//std::cout<<"Rigid Running "<<endl;	
		
		
		//instrumentX[0].getCenter() = posD[1].getCenter();// * Quat(Vector3(0,1,0),0.001) * Quat(Vector3(0,0,1),0.001);
		//cerr<<"\ncordinates\n"<<posD[1].getCenter();
		//////////float Z = posD[1].getOrientation()[2];
		//instrumentX[0].getCenter() = posD[1].getCenter()[2];//tampon.getOrientation();

		/////*std::cout<<"instrumentX[0].getOrientation() =  "<<instrumentX[0].getOrientation()<<std::endl;
		////std::cout<<"instrumentX[0].getTranslation() =  "<<instrumentX[0].getTranslation()<<std::endl;
