...
if ($_SESSION["userId"] != "") {
	...
	$catFb = array();
	$catLi = array();
	$query = "SELECT * FROM user_friend_detail WHERE userId = '" . $_SESSION['userId'] . "' ORDER BY FriendFirstName ASC, FriendLastName ASC limit 0, 25";
	$result = $mysql -> query($query);
		...
		while ($row = $result -> fetch_assoc()) {
			if ($row['ViewableRow'] != '0') {
				...
				if (!in_array($row["friendId"], $_SESSION["incatusers"]) || $_SESSION["incatusers"] == "") {
                    ...
                    <div class='friendsel_minf' <b>Activity:</b><br />News stories: ";	                     
	...
	$catFb = array();
	$catLi = array();
	$query = "SELECT * FROM user_friend_cat,user_detail_public WHERE user_friend_cat.userId = '" . $_SESSION['userId'] . "' AND catId='". $_GET["catid"] ."' AND user_detail_public.userId=user_friend_cat.friendId ORDER BY firstName ASC limit 0,25";
	//echo $query;
	//$query = "SELECT * FROM user_friend_cat WHERE userId = '" . $_SESSION['userId'] . "' AND catId='". $_GET["catid"] ."' limit 0,25";
	...
	if ($result -> num_rows > 0) {
		...
		while ($row2 = $result -> fetch_assoc()) {
			if ($row['ViewableRow'] != '0') {
				$query2 = "SELECT * FROM user_friend_detail WHERE userId = '" . $_SESSION['userId'] . "' AND friendId='". $row2["friendId"] ."' Order by FriendFirstName";
