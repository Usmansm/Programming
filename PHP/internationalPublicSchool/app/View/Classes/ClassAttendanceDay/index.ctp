...
$this->start('tabBody');
    echo $this->element('../Classes/ClassAttendanceDay/datepicker');
?>
                    ...
                    <?php
                        for($i = 0; $i < $dateDiff; $i ++){
                            for($d = 0; $d < $numOfSegment; $d ++){
                                ...
                                $link = $this->Html->link('<i class="fa fa-edit"></i>', array('controller'=> $this->params['controller'], 'action'=> 'ClassAttendanceDay/edit',$tableDate), array('target' => '_self','escape' => false));
                                echo '<th>'.$link.'</th>';
                            }
                        }
                    ?>
            ...
            <tbody>
                <?php
                    foreach($data as $student){ 
                        $studentAttendanceData = !empty($attendanceData)?$attendanceData['ClassAttendanceDay'][$student['Student']['id']]: array();
                ?>
                <tr>
