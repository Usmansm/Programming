	...
	public function index() {
		...
		foreach ($data as $key => $value) {
			$data[$key]['StudentData'] = $Student->find(
				...
				array(
					'recursive' => 0,
					'fields' => array('SecurityUser.id', 'SecurityUser.first_name', 'SecurityUser.middle_name', 'SecurityUser.last_name', 'SecurityUser.middle_name', 'SecurityUser.identification_no', 'SecurityUser.date_of_birth', 'SecurityUser.gender'),
					'conditions' => array(
	...
	public function student_search() {
		...
		if($this->controller->request->is('get')) {
			if($this->controller->request->is('ajax')) {
				...
				$result = $SecurityUser->autocomplete($search, 3);
