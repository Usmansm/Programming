...
if(@$_SESSION["userId"] != ""){
	...
	$query = "SELECT * FROM user_friend_detail WHERE userId = '".$_SESSION['userId']."' ORDER BY FriendFirstName ASC limit 0,25";
...
echo 	"<a name='". $row["friendId"] ."'></a>
			<div id=\"". $row["friendId"] ."\" class=\"friendsel_pic\" style=\"cursor: pointer;background: url('". DispFreProfilePic($row["friendId"]) ."');background-size: 100% 100%;\" >
				...
				<span onclick=\"get_friend_detail('". $row["friendId"] ."')\" >". $row["FriendFirstName"] ."</span>
				</div>
			</div>";
			}
		}
