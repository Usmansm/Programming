...
class ClassTeacher extends AppModel {
	...
	public function sort($conditions, $by, $direction) {
		$options = array(
			'recursive' => -1,
			'fields' => array(
				'ClassTeacher.id', 'Staff.id', 'Staff.staff_no',
				'SecurityUser.identification_no', 'SecurityUser.first_name', 'SecurityUser.middle_name', 'SecurityUser.last_name'
			),
			'joins' => array(
				array(
					'table' => 'staff',
	...
	public function autocomplete() {
		...
		if($this->request->is('ajax')) {
			...
			$list = $this->Staff->find('all', array(
				'recursive' => -1,
				'fields' => array('Staff.id', 'SecurityUser.first_name', 'SecurityUser.last_name', 'SecurityUser.identification_no'),
				'joins' => array(
					array(
						'table' => 'security_users',
						'alias' => 'SecurityUser',
	...
	public function getTeacherByClass($classId, $searchMode = 'all') {
		...
		//if($searchMode == 'all'){
			$options['fields'] = array(
				...
				'Staff.id', 
				'Staff.staff_no', 
				'SecurityUser.identification_no', 
		...
		if($searchMode == 'list'){
			...
			foreach($data as $obj) {
				...
				$list[$id] = sprintf('%s - %s %s', $staff_no, $first_name, $last_name);
			}
			return $list;
		}
		else{
	...
	public function getTeachersById($id) {
		$data = $this->find('all', array(
			'fields' => array('Staff.id', 'Staff.staff_no', 'ClassTeacher.id', 'ClassTeacher.staff_id', 'SecurityUser.identification_no',  'SecurityUser.first_name', 'SecurityUser.last_name',
