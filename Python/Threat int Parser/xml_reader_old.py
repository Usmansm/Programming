...
def main(feeds):
    ...
    for feed in feeds:
        ...
        ws.title = "Indicators"
        ws.append(('Indicator_id', 'Title', 'Type(s)', 'Description', 'Observable', 'Indicated_TTP', 'Producer'))  # , 'Source_FEED'))

        ws2 = workbook.create_sheet()
        ws2.title = 'TTPs'
        ws2.append(('TTP_Id', 'TTP title', 'Type', 'Name'))
        ...
        for event, indicator in context:
            ...
            indicator_type = ','.join([x.text for x in indicator_type_element]) if indicator_type_element else ''
            ...
            ws.append((indicator_id, indicator_title, indicator_type, indicator_description,
                       indicator_observable, indicator_indicated_TTP, indicator_producer))
        ...
        for event, ttp in context:
            ...
            ttp_name_element = ttp.find('.//ttp:Name', namespaces=namespaces)
            ttp_name = ttp_name_element.text if ttp_name_element is not None else 'NA'

            ws2.append((ttp_id, ttp_title, ttp_types, ttp_name))
