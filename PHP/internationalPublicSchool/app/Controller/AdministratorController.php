			...
			if(!in_array($this->action, array('view','add','search'))) {
				...
				if ($this->Session->check('Administrator.id') && $this->Session->read('Administrator.id')!=AuthComponent::user('id')) {
					...
					if(!in_array($this->action, array('view'))) { 
						$this->Navigation->addCrumb($name, $viewUrlArray);
					}
				} else {	
	...
	public function search() {
		$data = $this->SecurityUser->find(
			...
			array(
				'recursive' => -1,
				'fields' => array(
					'SecurityUser.first_name','SecurityUser.middle_name','SecurityUser.last_name','SecurityUser.id' 
				),
				'joins' => array(
					array(
	...
	public function edit($id=0) {
		...
		if ($this->SecurityUser->exists($userId)) {
			...
			if ($this->request->is(array('post', 'put'))) {
				// file upload part...
				$this->request->data['SecurityUser']['file'] = $this->request->data['Administrator']['photo_content'];
				unset($this->request->data['Administrator']['photo_content']);
