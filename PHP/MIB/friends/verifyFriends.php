...
foreach($multipleNames as $name){
	$query = "SELECT * FROM user_friend_detail WHERE FriendFirstName = '".$name[0]."' AND	FriendLastName = '".$name[2]."' AND userId = '".$_SESSION['userId']."'";
	$results = mysqli_query($conn, $query);
	...
	if($num == 2){
		...
		$data[0]['image'] = $raw["sourceProfileLink"];
		}
		if($raw2['sourceName'] == 'linkedin'){
		$data[1]['image'] = $raw2["sourceProfileLink"];
		}
