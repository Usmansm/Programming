...
class ClassLesson extends AppModel {
    ...
    function beforeAction() {
        ...
        $EducationGradesSubject = ClassRegistry::init('EducationGradesSubject');
        $this->fields['education_grade_subject_id']['type'] = 'select';
        $this->fields['education_grade_subject_id']['options'] = $EducationGradesSubject->getSubjectsWithEducationSubjectIdByClass($id);
    ...
    public function getTeachersList($classId, $educationSubjectId) {
        ...
        $options['recursive'] = -1;
        $options['fields'] = array('Staff.id', 'Staff.staff_no', 'SecurityUser.first_name', 'SecurityUser.middle_name', 'SecurityUser.last_name', 'SecurityUser.identification_no');
        $data = $this->find('all', $options);
    ...
    public function getTeachersListByClass($classId) {
        ...
        $options['group'] = array('ClassLesson.staff_id');
        $options['recursive'] = -1;
        //$options['fields'] = array('Staff.id','Staff.staff_no','SecurityUser.first_name','SecurityUser.last_name','SecurityUser.identification_no');
        $data = $this->find('all', $options);
    ...
    public function index($param1=null,$param2=null,$param3=null) {
        ...
        $EducationGradesSubject = ClassRegistry::init('EducationGradesSubject');
        $educationGradeSubjectOptions = $EducationGradesSubject->getSubjectsWithEducationSubjectIdByClass($id);
        ...
        if (isset($param2) && isset($param3)) {
            ...
            $this->controller->request->data = array('subject_id' => $param2, 'staff_id' => $param3);
        }
        else{
            $this->controller->request->data = array('subject_id' => 0, 'staff_id' => 0);
        }
        ...
        $this->controller->set('classId', $id);
        $this->controller->set('educationGradeSubjectOptions', $this->controller->Utility->getSetupOptionsData($educationGradeSubjectOptions, array('allField' => $this->Message->getLabel('ClassLesson.allSubjects'))));
        $this->controller->set('roomOptions', $roomOptions);
        $this->controller->set('staffOptions', $this->controller->Utility->getSetupOptionsData($staffOptions, array('allField' => $this->Message->getLabel('ClassLesson.allTeachers'))));
        $this->controller->Session->write('Class.lessonData', $lessonData);
        if(empty($lessonData)) $this->Message->alert('general.view.noRecords');
        $this->controller->set('data', $lessonData);
        $this->controller->set('startDate', $weekStart);
        $this->controller->set('endDate', $endtime[0]);
        $this->controller->set('model', $this->alias);
        $this->controller->request->data['ClassLesson']['startDate'] = $weekStart;
        $this->controller->request->data['ClassLesson']['endDate'] = $endtime[0];
    }
